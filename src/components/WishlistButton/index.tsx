...
const WishlistButton = (bookId: TBookId & { isActive?: string }) => {
  const { id, isActive } = bookId
  const { toggleWishlistItem } = useContext(WishlistContext);

  return (
    <Section onClick={() => toggleWishlistItem && toggleWishlistItem(id)}>
      <SVG width="24" height="24" fillRule="evenodd" clipRule="evenodd" fill={isActive} stroke={"#213547"}>
